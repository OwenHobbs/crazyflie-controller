...
ground_object_name = '2026_Holder_1'
basestation_name = '2026_BaseStation_1'
...
class DroneController:
    ...
    def _watch_for_key_press(self):
        ...
        while True:
            if keyboard.is_pressed("esc"):
                # self.stop_thread = Thread(target=self.stop_flying, daemon=True)
    ...
    def PID_control(self, x_pos, y_pos, z_pos, x_goal, y_goal, z_goal, Yaw, yaw_goal, times):
        ...
        new_yaw = (np.rad2deg(Yaw) + 360 ) % 360
        new_yaw_goal = (np.rad2deg(yaw_goal) + 360 ) % 360
        yaw_error = new_yaw_goal-new_yaw
        
        print(f"yaw goal: {new_yaw_goal}")
        print(f"yaw cur: {new_yaw}")
        print(f"x goal: {x_goal}")
        print(f"x cur: {x_pos}")
        print(f"y goal: {y_goal}")
        print(f"y cur: {y_pos}")
        print(z_goal)
        print(f"z cur: {z_pos}")
        if (yaw_error > 185):
        ...
        if (abs(yaw_error) > 3):
            yaw_rate = -60*(yaw_error/180) - 10*(yaw_error/abs(yaw_error))
        else:
            yaw_rate = 0.0
        ...
        if thrust > 2.3346:
            ...
            thrust = 0
        thrust = thrust*(50000.0/2.3346) + 10001.0
        # print(thrust)
        
        # Drone's z-axis points down, so flip the sign of the roll to compensate
        return (theta, -phi, yaw_rate, int(thrust))
    ...
    def _control_loop(self):
        ...
        x_goal, y_goal, z_goal, Yaw, _ = self._get_position_data(drone_object_name)
        ...
        while self.control:
            ...
            #TODO don't be ftns of time, instead once goal of step is hit move to next step ex once yaw = basestation, start to lower
            #TODO figure out xy coords of drone based on yaw/pos of hook
            timeTotal = 11
            base_correction = 0.4
            if RunTime < timeTotal - 9: #basestation hover
                x_goal, y_goal, z_base, yaw_goal, _ = self._get_position_data(basestation_name)
                yaw_goal += base_correction
                z_goal = z_base + 1.4
            elif RunTime < timeTotal - 7: #go to holder
                x_goal, y_goal, z_box, yaw_goal, _ = self._get_position_data(ground_object_name)
                yaw_goal = yaw_goal + math.pi/2
                z_goal = z_box + 1.3
            elif RunTime < timeTotal - 5: #lower to hook
                x_goal, y_goal, z_box, yaw_goal, _ = self._get_position_data(ground_object_name)
                z_goal = z_box + -1/7*math.pow((RunTime-(timeTotal-5)), 2) + 1.4
            elif RunTime < timeTotal: #back to basestation
                x_goal, y_goal, z_base, yaw_goal, _ = self._get_position_data(basestation_name)
                yaw_goal += base_correction
                x_goal -= 0.006
                z_goal = z_base + 1.4
            else: #lower to land TODO: don't start lowering until yaw within 5 degrees
                x_goal, y_goal, z_base, yaw_goal, _ = self._get_position_data(basestation_name)
                yaw_goal += base_correction
                x_goal -= 0.006
                z_goal = z_base + -1/7*math.pow((RunTime-timeTotal), 2) + 1.4
                if (z_goal < 0.3):
                    self.control = False
            ...
            pitch, roll, yaw_rate, thrust = self.PID_control(x_pos, y_pos, z_pos, x_goal, y_goal, z_goal, Yaw, yaw_goal, times)
            
            # print(f"yawrate: {yaw_rate}")
