...
class MissionControl:
    def __init__(self, flight_service, stop_event):
    ...
    def time_control(self, x_goal, y_goal, z_goal, yaw_goal, x_start, y_start, z_start, start_time): 
        ...
        print(f"z goal: {z_real}")

        if ((abs(drone_pos.x - x_goal) < 0.005) and (abs(drone_pos.y - y_goal) < 0.005) and (abs(drone_pos.z - z_goal) < 0.005) and (abs(drone_pos.yaw - yaw_goal) < 0.005)):
    ...
    def go_to(self, x_goal, y_goal, z_goal, yaw_goal): #right now this blocks until goal is reached
        ...
        while not goal_reached:
            if self.stop_event.is_set():
                return
            x_pos, y_pos, z_pos, yaw_pos, goal_reached = self.time_control(x_goal, y_goal, z_goal, yaw_goal, drone_pos.x, drone_pos.y, drone_pos.z, drone_pos.timestamp)
            self.flight_service.set_goal(Goal(x=x_pos, y=y_pos, z=z_pos, heading=yaw_pos))
            time.sleep(0.01)

    def hover(self, height):
        drone_pos = self.flight_service.get_latest_pose(self.flight_service._drone_object_name)
        self.go_to(drone_pos.x, drone_pos.y, drone_pos.z + height, drone_pos.yaw)
    ...
    def follow_object(self, object_name, offset_x, offset_z, offset_yaw):
        ...
        self.go_to(object_pos.x + offset_x, object_pos.y, object_pos.z + offset_z, object_pos.yaw + offset_yaw)

    def return_to_base(self, base_name):
        object_pos = self.flight_service.get_latest_pose(base_name)
        self.go_to(object_pos.x, object_pos.y, object_pos.z + 1.0, object_pos.yaw)
        self.go_to(object_pos.x, object_pos.y, object_pos.z + 0.003, object_pos.yaw)
